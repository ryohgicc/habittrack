...
### v1.0.9
- 🏷️ **优化**：插件名称统一为“四象限专注计时”，同步更新扩展名称、浏览器标题和弹窗标题。
- 🐞 **修复**：修复浏览器沉睡窗口在次日恢复后状态不同步的问题。现在页面重新可见、聚焦或从缓存恢复时会主动拉取最新状态，自动结束任务等进度可即时同步，无需手动刷新。
- ✨ **新增**：设置页新增“定时提醒开始任务”功能（默认关闭），可配置提醒时间；到点仍未开始任何任务时弹出强提醒弹窗。
- 🎨 **优化**：重构悬浮窗视觉样式，统一浅色/深色主题、按钮、弹窗、统计面板和任务列表的设计细节。
- 📐 **优化**：增强悬浮窗拖拽、缩放和最小化状态的自适应表现，降低网页样式对插件界面的影响。
- 🖥️ **优化**：提醒改为只投递给当前激活的网页标签，关闭后其他页面不会补弹。
- 🐞 **修复**：补充扩展上下文失效时的保护逻辑，减少旧页面在扩展重载后的报错。

### v1.0.8
- ✨ **新增**：设置页新增“自动休息”开关（默认关闭），支持配置午休开始时间与晚休开始时间，到点后自动结束进行中任务并切换为休息计时。
- 🔒 **优化**：任务“完成”操作增加二次确认弹窗，降低误触导致的误完成风险。
